...
async function scanDevices() {
  ...
  for (const d of res.devices) {
    if (devicesMap.has(d.id)) {
      ...
      newMap.set(d.id, { name: d.id, connected: false, info: null, tkConfig: { side: 'auto', lacs: [] } });
    }
  }
  devicesMap = newMap;
  renderTable();
...
function renderTable() {
  ...
  for (const [id, dev] of devicesMap.entries()) {
    ...
    tdSettings.style.textAlign = 'center';
    const btnSettings = document.createElement('button');
    btnSettings.innerText = '⚙️ Cài đặt';
    btnSettings.className = 'action-btn';
    btnSettings.onclick = () => openTkSettingsModal(id, dev);
...
// Modal Logic
const modal = document.getElementById('tk-settings-modal');
let currentEditingDevId = null;

function openTkSettingsModal(id, dev) {
  currentEditingDevId = id;
  const cfg = dev.tkConfig || { side: 'auto', lacs: [] };
  document.getElementById('modal-side-select').value = cfg.side || 'auto';
  document.getElementById('modal-lac-1').value = cfg.lacs[0] || 'none';
  document.getElementById('modal-lac-2').value = cfg.lacs[1] || 'none';
  document.getElementById('modal-lac-3').value = cfg.lacs[2] || 'none';
...
document.getElementById('modal-btn-save').addEventListener('click', () => {
  if (!currentEditingDevId) return;
  const dev = devicesMap.get(currentEditingDevId);
  if (dev) {
    const side = document.getElementById('modal-side-select').value;
    const l1 = document.getElementById('modal-lac-1').value;
    const l2 = document.getElementById('modal-lac-2').value;
    const l3 = document.getElementById('modal-lac-3').value;
    dev.tkConfig = { side, lacs: [l1, l2, l3].filter(x => x !== 'none') };
    updateGlobalTK();
  }
  modal.style.display = 'none';
});
