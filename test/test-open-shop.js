...
async function run() {
    ...
    if (targetCid) {
        console.log("Connected successfully! Injecting shop opener script for CID: " + targetCid + "...");
    } else {
        console.log("Connected successfully! Scanning for nearby shop & sending open packet...");
    }
    ...
    const scriptCode = `
        ...
        function doOpenShop(targetCid) {
            try {
                ...
                if (!il2cppBase) {
                    send({ type: 'result', data: { ok: true, sentBytes: ret, cid: targetCid, fd: gameFd, payload: str, uiTriggered: false, error: 'libil2cpp.so base not found' } });
                    return;
                }
                
                var talkFnAddr = il2cppBase.add(0xE45040);
                
                var fn_string_new = null;
                var fn_domain_get = null;
                var fn_thread_attach = null;
                
                // Helper to find exports natively if Module.findExportByName fails
                function findExport(base, name) {
                    var ext = Module.findExportByName('libil2cpp.so', name);
                    if (ext) return ext;
                    // Nasty fallback if needed, but normally findExportByName works if module is loaded.
                    return null;
                }
                
                fn_string_new = Module.findExportByName('libil2cpp.so', 'il2cpp_string_new');
                fn_domain_get = Module.findExportByName('libil2cpp.so', 'il2cpp_domain_get');
                fn_thread_attach = Module.findExportByName('libil2cpp.so', 'il2cpp_thread_attach');
                
                if (fn_string_new && fn_domain_get && fn_thread_attach) {
                    var get_domain = new NativeFunction(fn_domain_get, "pointer", []);
                    var attach = new NativeFunction(fn_thread_attach, "pointer", ["pointer"]);
                    attach(get_domain());
                    ...
                    send({ type: 'result', data: { ok: true, sentBytes: ret, cid: targetCid, fd: gameFd, payload: str, uiTriggered: false, error: 'il2cpp exports not found' } });
                }
            } catch(e) {
        ...
        doOpenShop('${targetCid || ""}');
    `;
    
    const fs = require('fs');
    fs.writeFileSync('test/temp-open-shop.js', scriptCode);
    ...
    await frida.loadScript('test/temp-open-shop.js');
}
