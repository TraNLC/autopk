...
async function run() {
    ...
    console.log("Injecting script to read Bagarate from MainCanvas...");
    ...
    fs.writeFileSync('test/temp-scan-bagarate.js', scriptCode);
    
    frida.onMessage(m => {
        if (m.type === 'result') {
            if (m.data.ok) {
                console.log("\\n=== BAGARATE ITEMS (" + m.data.items.length + ") ===");
                fs.writeFileSync('bagarate_output.json', JSON.stringify(m.data.items, null, 2));
                console.log("Dumped to bagarate_output.json");
            } else {
                console.error("Error:", m.data.error);
            }
            process.exit(0);
        } else if (m.log) {
            console.log(m.log);
    ...
    await frida.loadScript('test/temp-scan-bagarate.js');
}
